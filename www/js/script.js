...
{   
    document.addEventListener('backbutton', function(e){
        ...
        if(activePage == 'act-page' || activePage == 'auth-page')
        {
            e.preventDefault();
...
$(document).on("pageshow", "#point-page", function () {
    function loadpointsform(point_list) {
        ...
        $.each(p_list, function (key) {
            point =
                '<li  data-corners="false" data-shadow="false" data-iconshadow="true" data-wrapperels="div" data-icon="false" data-theme="c" point="' + p_list[key].id + '"  class="ui-btn  ui-li ui-li-has-alt ui-li-has-thumb ui-btn-up-c ui-li-static ui-body-c ui-first-child item-list-point">' +
...
var plugin_vk = {
    ...
    auth: function (force) {
        if (!window.localStorage.getItem("plugin_vk_token") || force || window.localStorage.getItem("plugin_vk_perms")!=plugin_vk.plugin_perms) {
            var authURL="https://oauth.vk.com/authorize?client_id=4108857&scope="+this.plugin_perms+"&redirect_uri=http://oauth.vk.com/blank.html&display=touch&response_type=token&display=mobile";
